...
namespace KnxTunnelSS
{
    internal class KnxHelper
    {
        ...
        public static int GetDataLength(bool b_bit,byte[] data)
        {
            if (data.Length <= 0)
                return 0;
        ...
        public static void WriteData(byte[] datagram, bool b_bit, byte[] data, int dataStart)
        {
            if ((data.Length>0) && b_bit)
            {
                datagram[dataStart] = (byte)(datagram[dataStart] | (data[0]&0x3F));
            }
